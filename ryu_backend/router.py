...
class RDCController(ControllerBase):
    ...
    def set_ocs(self, req, **kwargs):
        logger.RouterLog.debug("Set OCS")
        ip = kwargs['ip']
        ...
        logger.RouterLog.debug("REST OCS in port: %s", ocs_in_port)
        logger.RouterLog.debug("REST OCS out port: %s", ocs_out_port)
        if ocs_in_port is None or ocs_out_port is None:
            logger.RouterLog.error("Missing 'ocs_in_port' or 'ocs_out_port' in payload")
        ...
        if not isinstance(ocs_in_port, list) or not isinstance(ocs_out_port, list):
            logger.RouterLog.error("'ocs_in_port' and 'ocs_out_port' must be lists")
        ...
        if not all(isinstance(port, int) for port in ocs_in_port):
            logger.RouterLog.error("'ocs_in_port' must be a list of integers")
        ...
        if not all(isinstance(port, int) for port in ocs_out_port):
            logger.RouterLog.error("'ocs_out_port' must be a list of integers")
            return Response(status=400, body="'ocs_out_port' must be a list of integers")

        logger.RouterLog.debug("Successfully updated switch info for IP: %s", ip)

        try:
            self.rdc_app.run_OCS_create_initial_connections(ocs_in_port + ocs_out_port, ocs_out_port + ocs_in_port)
        except Exception as e:
            logger.RouterLog.Error("Error updating switch info for IP %s: %s", ip, e)
    ...
    def set_switch_info(self, req, **kwargs):
        ...
        try:
            ...
            logger.RouterLog.Error("Failed to parse JSON in set_switch_info")
        ...
        if host_ports is None or switch_ports is None:
            logger.RouterLog.error("Missing parameters: hostPorts=%s, switchPorts=%s", host_ports, switch_ports)
        ...
        if not (isinstance(host_ports, list) and isinstance(switch_ports, list)):
            logger.RouterLog.error("Parameters 'hostPorts' and 'switchPorts' must be lists")
        ...
        if not all(isinstance(p, int) for p in host_ports):
            logger.RouterLog.error("'hostPorts' must be a list of integers, got %s", host_ports)
            return Response(status=400, body="'hostPorts' must be a list of integers")
        if not all(isinstance(p, int) for p in switch_ports):
            logger.RouterLog.error("'switchPorts' must be a list of integers, got %s", switch_ports)
        ...
        try:
            ...
            logger.RouterLog.Error("Exception while setting switch info for dpid %d: %s", dpid, e)
            return Response(status=500, body='Internal Server Error while setting switch info')


        logger.RouterLog.debug("Switch ID: %d", dpid)
        logger.RouterLog.debug("Switch Host Ports: %s", host_ports)
        logger.RouterLog.debug("Switch Switch Ports: %s", switch_ports)
        try:
            # Reinstall flows with updated variables
            # self.rdc_app.update_flows()
            pass
        except Exception as e:
            logger.RouterLog.Error("Error while updating flows: %s", e)
    ...
    def create_forwarding_table(self, req, **kwargs):
        logger.RouterLog.info("Create Forwarding Table")
    ...
    def set_forwarding_table(self, cmd, req, **kwargs):
        ...
        logger.RouterLog.debug("Forwarding Table with IP: %s", forwardingTable)
